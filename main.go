...
func handleConnectionError(err error) {
	if strings.Contains(err.Error(), "connection refused") ||
		...
		fmt.Fprintf(os.Stderr, "  plccli --connection %s --service --endpoint opc.tcp://opc-ua-server-ip:4840 --username \"username\" --password \"password\"\n", *connection)
		os.Exit(1)
	}
	// For other errors
...
func main() {
	...
	if *version {
		...
		fmt.Printf("Commit: %s\n", buildCommit)
		fmt.Printf("Built: %s\n", buildTime)
		fmt.Printf("Copyright Octanis Instruments GmbH: %s\n", buildTime)
		os.Exit(0)
	}
